...
export default async function handler(req, res) {
  ...
  try {
    const { businessId, branchId, staffId, period, startDate, endDate, month, year } = req.query;
    ...
    // Build branch filter (optional for owners with multiple branches)
    let branchFilter = '';
    if (branchId) {
      branchFilter = `AND br.id = $${paramIndex}`;
      dateParams.push(branchId);
      paramIndex++;
    }
    ...
    const staffSummaryQuery = `
      WITH monthly_commissions AS (
        ...
        WHERE br.business_id = $1
          ...
          ${branchFilter}
          ${staffFilter}
        GROUP BY s.id, s.full_name, s.commission_percentage, br.branch_name, EXTRACT(MONTH FROM b.created_at), EXTRACT(YEAR FROM b.created_at)
        ...
        FROM staff s
        INNER JOIN branches br ON s.branch_id = br.id
        LEFT JOIN bookings b ON s.id = b.assigned_staff_id AND b.branch_id = br.id
        WHERE br.business_id = $1
          AND b.status = 'completed'
          ${dateFilter}
          ${branchFilter}
      ...
      GROUP BY mc.staff_id, mc.staff_name, mc.commission_percentage, mc.branch_name,
               cp.id, cp.payment_date, cp.payment_notes, cp.recorded_at
      ORDER BY mc.branch_name, mc.staff_name
    `;
    ...
    console.log('📊 Owner commission report generated:', {
      businessId,
    ...
    console.error('❌ Owner commission reports error:', error);
